...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	...
	sb.append(" select stageno from member where checks = 1 ");
	...
	alert("<%= rs.getInt(1) %>스테이지부터 클리어하세요");
...
<div class="game-container">
    ...
    <button onclick="bu()" class="btn btn-primary">돌아가기</button>
</div>
...
<script type="text/javascript">
 ...
 // 벽돌 설정
    const brickRowCount = 15;        // 각 열에 있는 벽돌의 개수
    const brickColumnCount = 6;     // 각 행에 있는 벽돌의 개수
    ...
    const brickOffsetLeft = 20;     // 첫 번째 벽돌이 시작하는 위치의 왼쪽 여백 (캔버스의 왼쪽으로부터 픽셀 단위)
                ...
                if (b.status == 1) {
                    if (
                        ...
                        if (score == brickRowCount * brickColumnCount) {
                            alert("당신은 현실을 넘어 세상으로 나왔습니다.");
                            window.location.href = "success.jsp?no=<%= no %>";
                        }
                    }
                }
            }
    ...
    function draw() {
        ...
        backgroundImage.src = 'images/map3.png'; // 배경 이미지의 경로를 설정합니다.
        drawBricks();
        drawBall();
        drawPaddle();
        drawScore();
